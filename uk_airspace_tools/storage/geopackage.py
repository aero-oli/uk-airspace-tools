...
from .schema import ATTR_FIELDS, LAYER_NAMES
...
class GeoPackageCache:
    ...
    def write(self, features: list[AirspaceFeature], metadata: dict | None = None) -> Path:
        try:
            from osgeo import gdal, ogr, osr
        except ImportError as exc:
            raise RuntimeError("GDAL/OGR is required to write the GeoPackage cache.") from exc
        ...
        polygon_layer = self._create_feature_layer(datasource, LAYER_NAMES["polygon"], ogr.wkbPolygon, spatial_ref)
        point_layer = self._create_feature_layer(datasource, LAYER_NAMES["point"], ogr.wkbPoint, spatial_ref)
        text_layer = self._create_feature_layer(datasource, LAYER_NAMES["text_only"], ogr.wkbNone, None)
        warning_layer = self._create_warning_layer(datasource)
        metadata_layer = self._create_metadata_layer(datasource)

        for feature in features:
            target = self._target_layer(feature, polygon_layer, point_layer, text_layer)
            self._write_feature(ogr, target, feature)
    ...
    def _create_feature_layer(self, datasource, name, geom_type, spatial_ref):
        from osgeo import ogr

        layer = datasource.CreateLayer(name, spatial_ref, geom_type)
        for field_name in ATTR_FIELDS:
            if field_name.endswith("_value") or field_name in {
                "lower_altitude_ft",
                "upper_altitude_ft",
                "radius_nm",
                "latitude",
    ...
    def _create_warning_layer(self, datasource):
        ...
        layer = datasource.CreateLayer(LAYER_NAMES["warnings"], None, ogr.wkbNone)
    ...
    def _create_metadata_layer(self, datasource):
        ...
        layer = datasource.CreateLayer(LAYER_NAMES["metadata"], None, ogr.wkbNone)
    ...
    @staticmethod
    def _target_layer(feature, polygon_layer, point_layer, text_layer):
        if feature.geometry_quality == "qline_radius":
            return polygon_layer
        if feature.geometry_quality == "qline_point":
            return point_layer
        return text_layer

    def _write_feature(self, ogr, layer, feature: AirspaceFeature) -> None:
    ...
    @staticmethod
    def _attribute_value(feature: AirspaceFeature, field_name: str):
